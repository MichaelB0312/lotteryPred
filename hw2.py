...
for file in os.listdir(universal):
    ...
    i = i+1
    plt.subplot(3,3,i)
    plt.scatter(x='genome_sim', y='gene_sim', data=df)
    lowest = sn.lowess(df['genome_sim'], df['gene_sim'], frac=0.2)
    plt.plot(lowest[:,0], lowest[:,1], color='red')
    slope, intercept, _, _, _ = linregress(df['genome_sim'], df['gene_sim'])
    reg_line = slope*df['genome_sim']+intercept
    plt.plot(df['genome_sim'].to_numpy(), reg_line.to_numpy(), color='orange')
    r2 = r2_score(df['genome_sim'], df['gene_sim'])
    plt.title(f'Gene name: {file}, adjusted R-squared: {r2}')
    plt.xlabel('genome_sim')
    plt.ylabel('gene_sim')
